...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	...
	<title><?= $clinic['user_clinic_name'];?></title>
...
?>
			...
			<div class="container-fluid">
				<div class="row">
					 <ol class="breadcrumb pull-right" style="font-size:12px;">
						  ...
						  <li class="breadcrumb-item"><a href="<?= base_url('findaclinic');?>">Find A Clinic</a></li>
						  <li class="breadcrumb-item active"><?= $clinic['user_clinic_name'];?></li>
					</ol>
    ...
    position: relative;">
				...
				<div class="col-md-3 col-sm-3 sidebar" style="">		 
					 <img class="img img-reponsive img-thumbnail img-rounded" <?php if($clinic['user_image']==''){ ?> src="<?= base_url('img/clinicdemo.jpg');?>" <?php } else { ?>src="<?= base_url('img/user/'.$clinic_id.'/'.$user_image);?>" <?php } ?> alt="<?= $clinic['user_clinic_name'];?>" style="width:100%;">
					<br>
					<br>
					<button onclick="callback('<?= $clinic_id; ?>','<?= $clinic['user_clinic_name'];?>');" class="btn btn-success btn-block">Request A Callback</button>

					<?php
						if(isset($_COOKIE['wish_user_id']))
						{
							?>
							<a href="#" onClick="bookanappointment('<?= $clinic['user_id'];?>','<?= $clinic['user_clinic_name'];?>')" class="btn btn-block btn-success">Book An Appointment</a>
							<?php
				...
				<div class="col-md-9 col-sm-9">
					<h3 style="font-size:40px;"><?= $clinic['user_clinic_name'];?></h3>
					<?php
					...
					?>
					<p><b>Location:</b>	<?= $clinic['user_city'];?> <?= $clinic['user_state'];?> </p>
				
					<?php
						if($clinic['user_service']!='')
						{
							?>
							<p><b>Service:</b>	<?= $clinic['user_service'];?> </p>
							<?php
						}
						if($clinic['user_clinic_website']!='')
						{
							?>
							<p><b>Webiste:</b>	<a href="<?= $clinic['user_clinic_website'];?>" target="_blank"><?= $clinic['user_clinic_website'];?> </a></p>
							<?php
						}
					?>
						...
						?>
						<h4><b>Treatments Offered:</b></h4>
						<table class="table table-responsive" style="font-size:14px;border:0px;">
							<tbody>
								<?php
									if($clinic['user_fertiity_treatments']!='')
									{
										?>
										<tr>
										<th>Fertility Treatments</th>
										<td><p class="text text-justify"><?= $clinic['user_fertiity_treatments'];?></p></td>
										</tr>
										<?php
									}
									if($clinic['user_infertility_investigation']!='')
									{
										?>
										<tr>
										<th>Infertility investigation</th>
										<td><p class="text text-justify"><?= $clinic['user_infertility_investigation'];?></p></td>
										</tr>
										<?php
									}
									if($clinic['user_storage']!='')
									{
										?>
										<tr>
										<th>Fertility Treatments</th>
										<td><p class="text text-justify"><?= $clinic['user_storage'];?></p></td>
										</tr>
										<?php
									}									
								?>							 
